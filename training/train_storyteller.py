...
prompt_style = "World Context: {}\nNarrative: {}"

def format_prompts(batch):
    return { "text": [prompt_style.format(c, n) for c, n in zip(batch["context"], batch["narrative"])] }

# Загрузка данных
import os
import json

file_path = "storyteller_data.jsonl"

if not os.path.exists(file_path):
    print(f"\n[!] ФАЙЛ {file_path} НЕ НАЙДЕН. Создаю примеры...")
    dummy_data = [{"context": "Forest at night", "narrative": "The trees groan..."}]
...
trainer = SFTTrainer(
    ...
    args=TrainingArguments(
        ...
        bf16=False,
        logging_steps=10,
        output_dir="outputs_storyteller",
...
trainer.train()
model.save_pretrained("storyteller_lora_adapter")
tokenizer.save_pretrained("storyteller_lora_adapter")

# --- КОПИРОВАНИЕ НА DRIVE ---
if IS_COLAB:
    import shutil
    drive_adapter_path = os.path.join(SAVE_PATH, "storyteller_lora_adapter")
    if os.path.exists(drive_adapter_path): shutil.rmtree(drive_adapter_path)
    shutil.copytree("storyteller_lora_adapter", drive_adapter_path)
